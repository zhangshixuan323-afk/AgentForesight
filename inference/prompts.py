...
def _build_user_prompt(turns: list[dict],
                       ...
                       *,
                       ...
                       tail_start: int = 0) -> tuple[str, dict]:
    ...
    def render(kf: int, kr: int, ptc: int | None, sc: int,
        ...
        if prior_summary is not None:
            # L2 mode: prior model summary of [0..tail_start-1] + verbatim tail.
            parts.append(f"[Prior summary of steps 0..{tail_start - 1} (generated by the auditor)]")
            parts.append(prior_summary)
            parts.append("")
            tail = partial[tail_start:]
            lines, n_compressed = _render_window(tail, 0, kr, ptc, sc)
            parts.extend(lines)
    ...
    # Verbatim first (L1 mode only): render everything in full, only compress
    # if over budget. L2 mode (prior_summary) is inherently non-verbatim and
    # goes straight to the windowed render.
    if prior_summary is None:
        full, _ = render(0, 0, None, summary_chars)
        est = _estimate_tokens(full, tokenizer)
        if est <= max_input_tokens:
            return full, {
                "verbatim": True, "compressed_turns": 0, "over_budget": False,
                "input_tokens_estimate": est, "max_input_tokens": max_input_tokens,
            }
    else:
        est = max_input_tokens + 1  # force the compression loop below
    ...
    # line. With allow_collapse=False (L2 mode) the escalation stops at the L1
    # floor and reports over_budget=True so the caller can switch to the
    # rolling model summary. Every step is deterministic and logged.
    ...
    while est > max_input_tokens:
        if kr > 2:
            ...
            sc = max(120, sc // 2)
        elif allow_collapse and not collapse:
            collapse = True                       # one-time middle collapse
        else:
            break                                 # floor: caller decides (L2)
        out, n_compressed = render(keep_first, kr, ptc, sc, collapse_middle=collapse)
        est = _estimate_tokens(out, tokenizer)

    return out, {
        "verbatim": False, "compressed_turns": n_compressed,
        "collapsed_middle": collapse, "over_budget": est > max_input_tokens,
        "prior_summary": prior_summary is not None, "tail_start": tail_start,
        "input_tokens_estimate": est, "max_input_tokens": max_input_tokens,
...
def build_user_prompt(turns: list[dict],
    ...
    text, _ = _build_user_prompt(
        ...
        allow_collapse=kwargs.pop("allow_collapse", True),
        prior_summary=kwargs.pop("prior_summary", None),
        tail_start=kwargs.pop("tail_start", 0),
...
def build_messages(turns: list[dict],
    ...
    ``stats`` records verbatim/compressed rendering, the token estimate and the
    ``over_budget`` flag (L2 mode)."""
    text, stats = _build_user_prompt(
        ...
        allow_collapse=allow_collapse, prior_summary=prior_summary,
...
# --- L2: rolling model summary (compression V2) ------------------------------

SUMMARY_SYSTEM_PROMPT = (
    "You are compressing an already-audited trajectory span into a concise "
    "factual summary for an auditor.\n"
    "Keep: agent identities and roles, tool calls with key arguments, decisive "
    "facts/values, and any error indicators already observed.\n"
    "Drop: verbatim tool output, repeated text, anything not needed to judge "
    "later steps.\n"
    "Output ONLY the updated summary (plain text, at most {budget} characters)."
)

def build_summary_messages(turns: list[dict],
                           span_start: int,
                           span_end: int,
                           *,
                           prior_summary: str | None = None,
                           budget_chars: int = 2000,
                           per_turn_chars: int = 12000) -> list[dict]:
    """Messages that produce the rolling summary of turns[0..span_end].

    ``prior_summary`` is the summary of turns[0..span_start-1]; only the new
    span turns[span_start..span_end] are rendered verbatim (per-turn capped),
    keeping the input bounded regardless of trajectory length.
    """
    lines: list[str] = []
    if prior_summary:
        lines.append(f"Existing summary (steps 0..{span_start - 1}):\n{prior_summary}")
    body = "\n".join(
        _render_turn(t, i, per_turn_chars)
        for i, t in enumerate(turns[span_start:span_end + 1], start=span_start)
    )
    lines.append(f"New turns (steps {span_start}..{span_end}):\n{body}")
    user = "\n\n".join(lines)
    user += f"\n\nWrite the updated summary covering steps 0..{span_end}."
    return [
        {"role": "system", "content": SUMMARY_SYSTEM_PROMPT.format(budget=budget_chars)},
